...
def reject_sample(R, F, N):
        ...
        while i < N:
                x = np.random.uniform(-R[-1], R[-1], 1)
                y = np.random.uniform(-R[-1], R[-1], 1)
                z = np.random.uniform(-R[-1], R[-1], 1)
                p = np.random.uniform(0, F[0], 1)
                u = np.interp((x*x+y*y+z*z)**0.5, R, F)
                if p < u:
                        samples[i] = [x, y, z]
                        i += 1
        return samples
...
def joint_reject_sample(R_p, F_p, R_d, F_d, N):
        ...
        while i < N:
                ...
                u = np.interp((x*x+y*y+z*z)**0.5, R_p, F_p) * np.interp((dx*dx+dy*dy+dz*dz)**0.5, R_d, F_d)

                mask = p < u
                if mask.sum() > 0:
                        samples[i:i+mask.sum()] = np.hstack([x[mask].reshape((-1,1)), 
                                y[mask].reshape((-1,1)), 
                                z[mask].reshape((-1,1)), 
                                dx[mask].reshape((-1,1)), 
                                dy[mask].reshape((-1,1)), 
